...
public class FSMQueryBuilder {
	...
	private static final String dashboardSearchQuery = "SELECT counts.completed, counts.progress, counts.feedback_pending, counts.driver_assigned, counts.pending, counts.Total"
			+ " FROM (SELECT COUNT(CASE WHEN fsm.applicationstatus != 'PENDING_FEE_PAYMENT' THEN 1 END) AS Total,"
	...
	public String getFSMDashboardCountQuery() {
